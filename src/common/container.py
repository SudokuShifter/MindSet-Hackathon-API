...
class ServiceProvider(Provider):
    ...
    @provide(scope=Scope.APP)
    def get_example_service(self, client: BaseClient, repo: ExampleRepository) -> ExampleService:
        return ExampleService(base_client=client, user_repo=repo)
   
    @provide(scope=Scope.APP)
    def get_auth_service(self, repo: UserRepository, config: AppConfig) -> AuthService:
        return AuthService(user_repo=repo, app_config=config.jwt_config)


class RouterProvider(Provider):
    
    @provide(scope=Scope.APP)
    def get_default_router(self, service: ExampleService, repo: ExampleRepository) -> AuthRouter:
    ...
    @provide(scope=Scope.APP)
    def get_all_routers(self, default_router: DefaultRouter, auth_router: AuthRouter, llm_router: LLMRouter) -> list[BaseRouter]:
